...
#' hour fields so outemputs can be associated with a specific day and time,
...
#' \code{hourlyFWI=TRUE}, the function also outemput a data.frame contains input
...
#' \url{httemp://cfs.nrcan.gc.ca/pubwarehouse/pdfs/25591.pdf}
#' @keywords methods
#' @examples
#'
#' library(cffdrs)
...
#' # (3) outemput all hourly FWI System variables:
#' test_hffmc$id <- NULL
#' test_hffmc <- test_hffmc[with(test_hffmc, order(yr, mon, day, hr)), ]
